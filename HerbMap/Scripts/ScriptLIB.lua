...
function LogToChatWWVT(valuedText, name)
  ...
  if not common.IsWString(sender) then
    sender = userMods.ToWString(sender)
  end
...
function getAllChildrensOfWidget(tab, widget, metod)
  ...
  for _, w in pairs(childrens) do
    ...
    if metod and w[metod] then
      tab1 = tab1 .. " proc." .. metod
    end
    LogInfo(tab1)
...
function testAllMainForms()
  ...
  for _, w in pairs(childrens) do
    if string.lower(w:GetName()) == "mainform" then
      getAllChildrensOfWidget("+", w)
    end
  end
end
...
function wtScale(w, scaleX, scaleY, andChildrens)
  ...
  if not w or (scaleX == 1 and scaleY == 1) then
    return
  end
  local place = w:GetPlacementPlain()
  ---LogInfo(w:GetName())
...
function wtMove(w, shiftX, shiftY, highShiftX, highShiftY)
  ...
  if highShiftX then
    place.highPosX = place.highPosX + highShiftX
  end
  if highShiftY then
    place.highPosY = place.highPosY + highShiftY
  end
  w:SetPlacementPlain(place)
end
...
function wtGetSizeX(w, p)
  ...
  --]]
  if not w then
    return ScreenSize.fullVirtualSizeX
  end
  if not p then
    p = w:GetPlacementPlain()
  end
...
function wtGetSizeY(w, p)
  ...
  --]]
  if not w then
    return ScreenSize.fullVirtualSizeY
  end
  if not p then
    p = w:GetPlacementPlain()
  end
...
function wtSetVal(w, v)
  local n = w:GetName()
  if string.sub(n, -4) == "_clr" and w.SetBackgroundColor then
    w:SetBackgroundColor(v)
    return
  end
  ...
  if false then
  elseif string.find(n, "_btn") then
    w:SetVal("button_label", ToWS(v)) ---LogToChat(n)
  elseif string.find(n, "_mnu") then
    w:SetVal("button_label", ToWS(v))
  elseif string.find(n, "_edl") or string.find(n, "_edn") then
    w:SetText(ToWS(v))
  elseif string.find(n, "_chk") then
    w:SetVariant(v and 1 or 0)
  elseif string.find(n, "_cch") then
    w:SetVariant(v and 1 or 0) --- сложный CheckCmplx
  elseif w.SetVal then
    w:SetVal("value", ToWS(v))
  else
    LogInfo("wtSetVal: unknown widgets type:" .. n)
    errr()
  end
end
...
function wtGetVal(w)
  local n = w:GetName()
  if string.find(n, "_chk") then
    return w:GetVariant() == 1
  elseif string.find(n, "_edl") then
    return ToWS(w:GetText())
  end
end
...
local function wtReSkin(wt, nameIn)
  ...
  if string.sub(name, -4) == "_frm" then
    if skin.texture then
      wt:SetBackgroundTexture(skin.texture)
    end
    if skin.color then
      wt:SetBackgroundColor(skin.color)
    end
  elseif string.sub(name, -4) == "_btn" then
    if skin.item_texture then
      wt:SetBackgroundTexture(skin.item_texture)
    end
    if skin.item_color then
      wt:SetBackgroundColor(skin.item_color)
    end
...
function WCD(descr, name, parent, place, show, skinOn)
  if not descr then
    LogInfo("ScriptLIB.lua/function WCD(): - Descriptor is nil...")
    return
  end
  local n
...
function W(s, wt)
  return getNamedChild(wt or mainForm, s, true)
end
--- выдаьт unchecked ребенка вbджета только на это уровне
function Wun0(s, wt)
  return (wt or mainForm):GetChildUnchecked(s, false)
end

function FromWS(str)
  if str then
    return userMods.FromWString(str)
  end
end
...
function exObj(tab, obj, meta, exclude)
  --- explore an object
  ...
  if type(obj) == "table" then
    ...
    for k, v in pairs(obj) do
      ...
      elseif k == "parent" or exclude and (type(exclude) == "table" and exclude[k] or exclude == k) then
        ...
        if "userdata" ~= type(k) then
          exObj(tab .. "." .. k, v, nil, exclude)
        else
          exObj(tab .. ".userdata", v, nil, exclude)
        end
      end
    end
    LogInfo(tab, "}")
...
--- meta=true  - выдает метаданные объекта
--- exclude = "filter string" or { filter_1 = 1, filter_2 = 1, ... }
function exObj2(val, obj, meta, exclude, level)
  --- explore an object
  ...
  if type(obj) == "table" then
    ...
    for k, v in pairs(obj) do
      ...
      elseif key == "parent" or exclude and (type(exclude) == "table" and exclude[key] or exclude == key) then
        ...
        if "userdata" ~= type(k) then
          exObj2("[\"" .. key .. "\"]", v, nil, exclude, level)
        else
          exObj2("[{userdata}]", v, nil, exclude, level)
        end
      end
    end
...
local function getVersionAO1()
  if false then
  elseif avatar.GetInventoryOverflowSize then
    return 3 --- 2.0.08.12 [3.08.2011]  Атолл
  elseif stateMainForm:GetChildUnchecked("ChatLog", false) then
    ---- 2.0.06.13 [26.05.2011]
    local w = stateMainForm:GetChildUnchecked("RollGreedNeed", false)
    if w and w:GetChildUnchecked("ItemSlot", true) then
      --- --- 2.0.08.12 [3.08.2011]  Атолл
      return 3
...
if avatar.GetItemInfo then
  _avaItem = true
end
if not _avaItem and itemLib.GetItemInfo then
  _itemLib = true
end
...
function GetItemResourceId(itemId)
  return avatar.GetItemResourceId(itemId)
end
...
function wtEFFSfuncAdd(wt, func)
  ...
  if not wtEFFS[ID] then
    wtEFFS[ID] = { wt = wt, effs = {}, funcs = {} }
  end
...
local function wtFinishEffect(wt, type)
  if false then
  elseif type == ET_FADE then
    wt:FinishFadeEffect()
  elseif type == ET_MOVE then
    wt:FinishMoveEffect()
  elseif type == ET_RESIZE then
    wt:FinishResizeEffect()
  elseif type == ET_TEXTURE_ROTATION then
    wt:FinishRotationEffect()
  elseif type == ET_TEXT_SCALE then
    wt:FinishTextScaleEffect()
  end
end
...
function wtDestroy(wt)
  ...
  if childID then
    ...
    childTab.funcs = {}
    for type in pairs(childTab.effs) do
...
function NormalizePlacement(wt, PlaceIn)
  ...
  if Place.posX + sizeX > sizeXparent then
    Place.posX = sizeXparent - sizeX
  end
  if Place.posY + sizeY > sizeYparent then
    Place.posY = sizeYparent - sizeY
  end
  if Place.posX < 0 then
    Place.posX = 0
  end
  if Place.posY < 0 then
    Place.posY = 0
  end
